...
use crate::read_buf::{FuseReadBuf, INIT_BUFFER_SIZE};
...
impl<FS: Filesystem> Session<FS> {
    ...
    pub fn new<P: AsRef<Path>>(
        ...
        let mut session = Session {
            ...
            proto_version: None,
            receive_buf_size: INIT_BUFFER_SIZE,
    ...
    pub fn run(self) -> io::Result<()> {
        ...
        if n_threads == 1 {
            let result = SessionEventLoop {
                thread_name: "fuser-0".to_owned(),
            ...
            .event_loop();
            Arc::get_mut(&mut filesystem)
                .expect("single receive loop finished")
                .destroy();
            return result;
        }
    ...
    fn handshake(&mut self) -> io::Result<()> {
        let mut buf = FuseReadBuf::new(INIT_BUFFER_SIZE);
        let buf = buf.as_mut();
        ...
        loop {
            ...
            #[cfg(target_os = "linux")]
            {
                // Extension-bearing creation requests keep the legacy allowance;
                // LayerFS does not negotiate these optional input extensions.
                self.receive_buf_size = if config
                    .requested
                    .intersects(InitFlags::FUSE_SECURITY_CTX | InitFlags::FUSE_CREATE_SUPP_GROUP)
                {
                    MAX_WRITE_SIZE + 4096
                } else {
                    crate::read_buf::receive_size(
                        config.max_write,
                        config.max_pages(),
                        page_size::get(),
                    )?
...
impl<FS: Filesystem> SessionEventLoop<FS> {
    fn event_loop(&self) -> io::Result<()> {
        ...
        let mut buf = FuseReadBuf::new(self.receive_buf_size);
        let buf = buf.as_mut();
        loop {
...
#[cfg(test)]
mod layerfs_session_tests {
    use super::*;
    use crate::{FileHandle, INodeNo, ReplyAttr};
    use std::io::{Read, Write};
    use std::os::unix::net::UnixStream;
    use std::sync::atomic::{AtomicUsize, Ordering};

    struct Probe {
        seen: Arc<Mutex<Option<thread::ThreadId>>>,
    ...
    impl Filesystem for Probe {
        fn init(&mut self, _request: &Request, config: &mut KernelConfig) -> io::Result<()> {
            ...
            config.set_max_readahead(1024 * 1024).unwrap();
            Ok(())
        }
        ...
        fn getattr(
            ...
            *self.seen.lock() = Some(thread::current().id());
    ...
    fn frame(opcode: abi::fuse_opcode, unique: u64, payload: usize) -> Vec<u8> {
        let mut bytes = Vec::new();
        bytes.extend_from_slice(&((40 + payload) as u32).to_ne_bytes());
        ...
        bytes.extend_from_slice(&geteuid().as_raw().to_ne_bytes());
        bytes.resize(40 + payload, 0);
        bytes
    }

    #[test]
    fn single_loop_dispatches_on_run_thread_and_destroys_exactly_once() {
        ...
        let peer = thread::spawn(move || {
            let mut init = frame(abi::fuse_opcode::FUSE_INIT, 3, 64);
            init[40..44].copy_from_slice(&7_u32.to_ne_bytes());
            init[44..48].copy_from_slice(&40_u32.to_ne_bytes());
            init[48..52].copy_from_slice(&(1024_u32 * 1024).to_ne_bytes());
            ...
            assert!((40..=80).contains(&size));
            let mut init_reply = vec![0; size - 16];
            kernel.read_exact(&mut init_reply).unwrap();
            assert_eq!(
                u32::from_ne_bytes(init_reply[20..24].try_into().unwrap()),
                1024 * 1024
            );

            kernel
                .write_all(&frame(abi::fuse_opcode::FUSE_GETATTR, 1, 16))
                .unwrap();
            let mut reply = [0; 16];
            ...
            kernel
                .write_all(&frame(abi::fuse_opcode::FUSE_DESTROY, 2, 0))
                .unwrap();
            kernel.read_exact(&mut reply).unwrap();
        ...
        .unwrap();
        #[cfg(target_os = "linux")]
        assert_eq!(session.receive_buf_size, 1024 * 1024 + 4096);
        let caller = thread::current().id();
        let result = session.run();
        peer.join().unwrap();
        result.unwrap();
        assert_eq!(*seen.lock(), Some(caller));
