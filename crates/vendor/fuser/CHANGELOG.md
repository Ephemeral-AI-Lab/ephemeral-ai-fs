## LayerFS maintained patch (unreleased)

- Avoid a redundant receive worker when a session uses one thread.
- Size Linux request buffers from INIT negotiation while retaining non-write request capacity.
