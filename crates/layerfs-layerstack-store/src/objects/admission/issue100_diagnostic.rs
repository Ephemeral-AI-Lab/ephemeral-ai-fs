...
fn original(commit: &str, path: &str) -> Option<Vec<u8>> {
    let output = Command::new("git")
        ...
        .args(["show", &format!("{commit}:{path}")]).output().unwrap();
    if output.status.success() { Some(output.stdout) } else {
        assert!(String::from_utf8_lossy(&output.stderr).contains("does not exist in"), "{}", String::from_utf8_lossy(&output.stderr));
        None
    }
}
fn reconstruct(records: &[Encoded], index: usize) -> Vec<u8> {
    ...
    assert!(chain.len() <= 9);
    assert!(chain.iter().map(|&i| records[i].raw_len + 23).sum::<usize>() <= 512 * 1024);
    assert!(chain.iter().map(|&i| records[i].frame.len() + 9 + usize::from(records[i].base.is_some()) * 32).sum::<usize>() <= 256 * 1024);
    let mut raw: Option<Vec<u8>> = None;
    for &i in chain.iter().rev() {
        let node = &records[i];
        let next = pack::small_decompress(&node.frame, node.raw_len, raw.as_deref()).unwrap();
        assert_eq!(ObjectId::for_bytes(&content::encode_small(&next).unwrap()), node.id);
        raw = Some(next);
    }
    raw.unwrap()
}
...
"#, EVIDENCE]).output().unwrap();
    ...
    for path in [
        ...
        for chained in [false, true] {
            let policy = if chained { "immediate-predecessor" } else { "retained-FULL-anchor" };
            ...
            for row in mapping.lines() {
                ...
                let Some(raw) = original(columns[2], path) else { prior = None; previous_len = 0; continue; };
                if raw.is_empty() || raw.len() >= content::SMALL_LIMIT {
                    println!("ineligible\t{policy}\t{path}\t{}\t{}\tbytes={}\tCDC-or-empty", columns[0], columns[1], raw.len());
                    prior = None; previous_len = raw.len(); continue;
                }
                ...
                if let Some(index) = records.iter().position(|r| r.id == id) {
                    prior = Some(index); previous_len = raw.len();
                    println!("CAS\t{policy}\t{path}\t{}\t{}\t{id:?}", columns[0], columns[1]);
                    continue;
                }
                let candidate = prior.map(|i| if chained { i } else { records[i].base.unwrap_or(i) });
                let eligible = candidate.filter(|&i| records[i].depth < 8 && records[i].closure + canonical.len() <= 512 * 1024);
                ...
                let alternative = prefix.as_deref().map(|base| encoder.compress(&raw, Some(base)).unwrap());
                drop(encoder);
                let encoding = start.elapsed().as_nanos();
                encode_ns += encoding;
                if let Some(frame) = &alternative {
                    assert_eq!(pack::small_decompress(frame, raw.len(), prefix.as_deref()).unwrap(), raw);
                }
                assert_eq!(pack::small_decompress(&full, raw.len(), None).unwrap(), raw);
                let delta_len = alternative.as_ref().map_or(0, Vec::len);
                let selected = eligible.filter(|&i| delta_len + 32 < full_len && records[i].encoded_closure + delta_len + 41 <= 256 * 1024);
                let (frame, depth, closure, encoded_closure) = if let Some(i) = selected {
                    ...
                    (frame, records[i].depth + 1, records[i].closure + canonical.len(), size)
                } else {
                    full_bytes += full.len() + 9 + 16;
                    let size = full.len() + 9;
                    (full, 0, canonical.len(), size)
                };
                records.push(Encoded { id, raw_len: raw.len(), frame: frame.into_boxed_slice(), base: selected, depth, closure, encoded_closure });
