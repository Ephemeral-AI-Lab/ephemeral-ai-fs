...
fn small_chain_depth_closure_integrity_and_exact_reuse() {
    for (length, last_delta) in [(8192, 8), (96 * 1024, 4)] {
        ...
        let original: Vec<u8> = f.db.reader().unwrap().query_row(
            "SELECT data FROM object_packs WHERE pack_id=?1", [location.pack], |r| r.get(0),
        ).unwrap();
        let mut corrupt = original.clone();
        let directory = 16 + location.group * 16;
        let start = u32::from_le_bytes(corrupt[directory..directory+4].try_into().unwrap()) as usize;
        assert_eq!(corrupt[start], 2);
        corrupt[start] = 1;
        f.db.reader().unwrap().execute("UPDATE object_packs SET data=?1 WHERE pack_id=?2", rusqlite::params![corrupt, location.pack]).unwrap();
        assert!(f.db.read_object_row(target.id).is_err());
        let known = f.db.object_locations(&[target.id]).unwrap();
        assert!(compare(&f.db, &known, &mut vec![(target.id, &target.bytes)], &mut ObjectInsertMetrics::default(), 0).is_err());
        f.db.reader().unwrap().execute("UPDATE object_packs SET data=?1 WHERE pack_id=?2", rusqlite::params![original, location.pack]).unwrap();
        assert_eq!(f.db.read_object_row(target.id).unwrap(), target.bytes);
        // Missing intermediate dependency cannot return plausible unchecked bytes.
        f.db.reader().unwrap().execute("DELETE FROM objects WHERE object_id=?1", [history[1].id.as_bytes().as_slice()]).unwrap();
        assert!(f.db.read_object_row(target.id).is_err());
    }
}
...
fn small_chain_schema8_nonpromoting_and_explicit_upgrade() {
    ...
    connection.execute_batch(crate::statements::schema::V8).unwrap();
    drop(connection);
    ...
    assert_eq!(f.db.reader().unwrap().pragma_query_value(None, "user_version", |r| r.get::<_, i64>(0)).unwrap(), 8);
    ...
    f.db = StoreDb::connect(&old).unwrap();
    assert!(f.db.small_chain_format());
    for object in [first, second, third] { assert_eq!(f.db.read_object_row(object.id).unwrap(), object.bytes); }
}

#[test]
fn compact_framing_multiple_groups_and_authenticated_locator_failures() {
    let f = Fixture::new();
    let objects: Vec<_> = [1, 8192, 131071].into_iter()
        .map(|length| small(&random().into_iter().cycle().take(length).collect::<Vec<_>>(), None))
        .collect();
    let prepared = f.prepare(objects.clone());
    assert_eq!(prepared.packs.len(), 1);
    assert_eq!(&prepared.packs[0][8..16], &[4, 0, 0, 0, 3, 0, 0, 0]);
    f.publish(prepared);
    for object in &objects { assert_eq!(f.db.read_object_row(object.id).unwrap(), object.bytes); }
    f.db.validate_small_packs().unwrap();
    let target = &objects[2];
    let location = f.db.object_locations(&[target.id]).unwrap()[&target.id];
    assert_eq!(location.group, 2);
    let original: Vec<u8> = f.db.reader().unwrap().query_row("SELECT data FROM object_packs WHERE pack_id=?1", [location.pack], |r| r.get(0)).unwrap();
    // The indexed length is an input to reconstruction, never an authentication oracle.
    f.db.reader().unwrap().execute("UPDATE objects SET canonical_length=canonical_length-1 WHERE object_id=?1", [target.id.as_bytes().as_slice()]).unwrap();
    assert!(f.db.read_object_row(target.id).is_err());
    f.db.reader().unwrap().execute("UPDATE objects SET canonical_length=canonical_length+1 WHERE object_id=?1", [target.id.as_bytes().as_slice()]).unwrap();
