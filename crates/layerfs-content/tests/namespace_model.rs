...
use layerfs_content::tree::directory::codec::{
    decode_directory_node, decode_directory_state, encode_directory_node, encode_directory_state,
    profile_id, DirectoryNodeV1,
};
...
use layerfs_content::tree::inode::codec::{
    decode_inode_table_node, encode_inode_table_node, InodeTableNodeV1,
};
...
fn directory_state_and_branch_summaries_are_authoritative() {
    ...
    let bad_state = DirectoryStateV1 {
        ...
        mapping_root: ObjectId::from_bytes(
            &layerfs_content::decode_bytes_object(&store.get(empty.0).unwrap()).unwrap()[53..85],
        )
        .unwrap(),
    };
...
fn inode_branch_summary_and_occupancy_are_authoritative() {
    ...
    let root = layerfs_content::tree::inode::InodeTableRoot(
        store
