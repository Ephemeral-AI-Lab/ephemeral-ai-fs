...
fn content(store: &Store, root: ObjectId, name: &str) -> Vec<u8> {
    ...
    filesystem::stream(store, root, &path(name), &mut bytes).unwrap();
    bytes
}
...
fn compact_logical_lifecycle_preserves_scopes_hardlinks_and_history() {
    ...
    let scope = filesystem::namespace(&store, genesis)
        .unwrap()
        .scope
        .unwrap();
    let original: Vec<_> = (0..131071).map(|n| (n % 251) as u8).collect();
    let first = filesystem::apply_changes(
        &mut store,
        genesis,
        &[
            ContentChange::Write {
    ...
    .root_id;
    let original_inode =
        filesystem::resolve(&store, first, &path("a"), &mut LogicalCounters::default())
            .unwrap()
            .inode;
    assert_eq!(
        filesystem::resolve(
    ...
    assert_eq!(
        filesystem::stat(&store, first, &path("alias"))
            .unwrap()
            .0
            .namespace_ref_count,
        2
    );
    let second = filesystem::apply_changes(
        &mut store,
        first,
        &[
            ContentChange::Write {
    ...
    let third = filesystem::apply_changes(
        &mut store,
        second,
        &[
    ...
    .root_id;
    assert_ne!(
        filesystem::resolve(&store, third, &path("a"), &mut LogicalCounters::default())
            .unwrap()
            .inode,
        original_inode
    );
    assert_eq!(
        filesystem::stat(&store, third, &path("sub/b"))
            .unwrap()
            .0
            .namespace_ref_count,
        1
    ...
    assert_eq!(
        filesystem::namespace(&store, third).unwrap().scope,
        Some(scope)
    );
    let mut changes = Vec::new();
    filesystem::diff_roots(&store, first, third, |change| {
        changes.push(change);
        Ok(())
    })
    .unwrap();
    ...
    let foreign = store
        .put(&layerfs_content::tree::directory::codec::encode_namespace_root(foreign).unwrap())
        .unwrap();
    ...
    assert!(
        filesystem::replace_paths_from_snapshot(&mut store, third, foreign, &[path("a")]).is_err()
    );
    let inode_table = filesystem::namespace(&store, third)
        .unwrap()
        .inode_table_root;
...
fn compact_reconciliation_merges_independent_content_and_metadata_edits() {
    ...
    let base = filesystem::apply_changes(
        &mut store,
        empty,
        &[ContentChange::Write {
            path: "file".into(),
    ...
    .unwrap()
    .root_id;
    let source = filesystem::set_mode(&mut store, base, &path("file"), 0o600)
        .unwrap()
        .root();
    let destination = filesystem::replace_range(
        &mut store,
        base,
        &path("file"),
        0,
    ...
    .unwrap()
    .root();
    let merged = filesystem::reconcile_roots(&mut store, base, source, destination)
        .unwrap()
        .unwrap()
        .root();
    assert_eq!(content(&store, merged, "file"), b"new bytes");
    assert_eq!(
        filesystem::stat(&store, merged, &path("file"))
            .unwrap()
            .0
            .metadata_root,
        filesystem::stat(&store, source, &path("file"))
            .unwrap()
            .0
            .metadata_root
    );
    ...
    let restored =
        filesystem::replace_paths_from_snapshot(&mut store, merged, base, &[path("file")]).unwrap();
