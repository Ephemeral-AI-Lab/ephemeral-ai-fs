use layerfs_content::file::rope::ObjectStore;
use layerfs_content::tree::directory::codec::*;
use layerfs_content::tree::directory::{DirectoryStateV1, SymlinkStateV1};
use layerfs_content::tree::inode::codec::*;
use layerfs_content::tree::inode::{InodeId, InodeKind, InodeRecordV1};
use layerfs_content::tree::metadata::codec::*;
...
use layerfs_content::{encode_bytes_object, CoreError, ObjectId};
...
fn directory_inode_and_metadata_nodes_are_strict_and_ordered() {
    ...
    let metadata = MetadataNodeV1::Leaf {
        ...
        entries: vec![layerfs_content::tree::metadata::MetadataEntryV1 {
            key: layerfs_content::tree::metadata::MetadataKey::new(
                "portable".into(),
                b"mode".to_vec(),
            )
            .unwrap(),
...
fn metadata_builder_keeps_only_bounded_tail_summaries() {
    ...
    layerfs_content::tree::metadata::visit_metadata_entries(&store, root, |entries| {
