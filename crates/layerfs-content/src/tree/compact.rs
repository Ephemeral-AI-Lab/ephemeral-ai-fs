...
use super::directory::codec::{decode_node_value, exact_value, finish_node, node_count,
    node_header, node_subtree_bytes, node_subtree_count, ordered, put_bytes, take,
    take_bytes, validate_node_header};
...
impl InodeSerial {
    ...
    pub fn from_inode_key(key: super::inode::InodeId) -> CoreResult<Self> {
        if key.0[..24] != [0; 24] { return Err(CoreError::InvalidRecord("compact inode key")); }
        Self::read(&key.0[24..])
    }
...
pub(crate) fn is_inode_table<S: crate::object::access::ObjectRead>(store: &S, root: ObjectId) -> CoreResult<bool> {
    store.with_authenticated_canonical(root, |bytes| Ok(crate::decode_bytes_object(bytes)?.starts_with(b"LFS6INT\0")))
}

enum InodeWalk {
    Node { id: ObjectId, expected: Option<(u8, InodeSerial)> },
    End { before: u64, count: u64 },
}
...
impl InodeCursor {
    pub(crate) fn new(root: ObjectId) -> Self {
        Self { stack: vec![InodeWalk::Node { id: root, expected: None }], leaf: Vec::new().into_iter(), count: 0, previous: None }
    }
    pub(crate) fn next<S: crate::object::access::ObjectRead>(&mut self, store: &S, counters: &mut super::inode::InodeTableCounters) -> CoreResult<Option<(InodeSerial, InodeRecordV1)>> {
        loop {
            if let Some(row) = self.leaf.next() {
                if self.previous.is_some_and(|previous| previous >= row.0) { return Err(CoreError::NonCanonicalOrdering); }
                self.previous = Some(row.0);
                self.count = self.count.checked_add(1).ok_or(CoreError::LengthOverflow)?;
                return Ok(Some(row));
            }
            let Some(item) = self.stack.pop() else { return Ok(None); };
            let (id, expected) = match item {
                InodeWalk::Node { id, expected } => (id, expected),
                InodeWalk::End { before, count } => {
                    if self.count.checked_sub(before) != Some(count) { return Err(CoreError::InvalidRecord("compact inode subtree count")); }
                    continue;
                }
            };
            counters.nodes_read = counters.nodes_read.checked_add(1).ok_or(CoreError::LengthOverflow)?;
            ...
            let (level, maximum, count) = match &node {
                InodeNode::Leaf(rows) => (0, rows.last().ok_or(CoreError::NonCanonicalPagePartition)?.0, rows.len()),
                InodeNode::Branch { level, children, .. } => (*level, children.last().ok_or(CoreError::NonCanonicalPagePartition)?.0, children.len()),
            };
            if expected.is_some_and(|value| value != (level, maximum)) { return Err(CoreError::InvalidRecord("compact inode child summary")); }
            if expected.is_some() && count < if level == 0 { 50 } else { 64 }
                || level > 0 && count < 2 { return Err(CoreError::NonCanonicalPagePartition); }
            match node {
                InodeNode::Leaf(rows) => self.leaf = rows.into_iter(),
                InodeNode::Branch { level, subtree_count, children } => {
                    if self.stack.len() + children.len() + 1 > 32 * 128 { return Err(CoreError::ObjectLimitExceeded); }
                    self.stack.push(InodeWalk::End { before: self.count, count: subtree_count });
                    self.stack.extend(children.into_iter().rev().map(|(maximum, id)| InodeWalk::Node { id, expected: Some((level - 1, maximum)) }));
                }
            }
        }
    }
...
pub(crate) fn inode_lookup_from_node<S: crate::object::access::ObjectRead>(
    ...
    loop {
        ...
        if let Some((expected_level, expected_maximum)) = expected {
            ...
            if count < if level == 0 { 50 } else { 64 } { return Err(CoreError::NonCanonicalPagePartition); }
        } else if level > 0 && count < 2 { return Err(CoreError::NonCanonicalPagePartition); }
        if key > maximum { return Ok(None); }
        match node {
            InodeNode::Leaf(rows) => return Ok(rows.binary_search_by_key(&key, |row| row.0).ok().map(|index| rows[index].1)),
            InodeNode::Branch { level, children, .. } => {
                let index = children.partition_point(|row| row.0 < key);
                let (maximum, child) = children[index];
                expected = Some((level.checked_sub(1).ok_or(CoreError::MappingDepthExceeded)?, maximum));
                counters.nodes_read = counters.nodes_read.checked_add(1).ok_or(CoreError::LengthOverflow)?;
...
pub enum InodeNode {
    ...
    Branch { level: u8, subtree_count: u64, children: Vec<(InodeSerial, ObjectId)> },
}
...
pub fn decode_inode_value(bytes: &[u8]) -> CoreResult<InodeRecordV1> {
    if bytes.len() != 73 { return Err(CoreError::InvalidRecord("inline inode length")); }
    Ok(InodeRecordV1 { kind: InodeKind::try_from(bytes[0])?,
        namespace_ref_count: u64::from_be_bytes(bytes[1..9].try_into().unwrap()),
        content_root: ObjectId::from_bytes(&bytes[9..41])?, metadata_root: ObjectId::from_bytes(&bytes[41..])? })
}
...
pub fn encode_inode(node: &InodeNode) -> CoreResult<Vec<u8>> {
    let (role, level, count, total) = match node {
        ...
        InodeNode::Branch { level, subtree_count, children } => (8, *level, children.len(), *subtree_count),
    };
    validate_node_header(level, count)?;
    if count == 0 || count > if role == 7 { 100 } else { 127 } || (role == 8 && (level == 0 || total < count as u64)) {
        return Err(CoreError::NonCanonicalPagePartition);
    }
    let mut value = node_header(b"LFS6INT\0", role, level, count, total,
        total.checked_mul(81).ok_or(CoreError::LengthOverflow)?)?;
    match node {
        InodeNode::Leaf(rows) => {
            if rows.windows(2).any(|pair| pair[0].0 >= pair[1].0) { return Err(CoreError::InvalidRecord("compact inode key order")); }
        ...
        InodeNode::Branch { children, .. } => {
            if children.windows(2).any(|pair| pair[0].0 >= pair[1].0) { return Err(CoreError::InvalidRecord("compact inode key order")); }
...
pub fn decode_inode(canonical: &[u8]) -> CoreResult<InodeNode> {
    ...
    let node = match value[10] {
        7 if level == 0 && total == count as u64 => {
            if count == 0 || count > 100 || value.len() != 31 + count * 81 { return Err(CoreError::InvalidRecord("compact inode leaf length")); }
            let mut rows = Vec::with_capacity(count);
            for row in value[31..].chunks_exact(81) {
                rows.push((InodeSerial::read(&row[..8])?, decode_inode_value(&row[8..])?));
            }
            InodeNode::Leaf(rows)
        }
        8 if level > 0 => {
            if count == 0 || count > 127 || value.len() != 31 + count * 40 { return Err(CoreError::InvalidRecord("compact inode branch length")); }
            let mut children = Vec::with_capacity(count);
            for row in value[31..].chunks_exact(40) {
                children.push((InodeSerial::read(&row[..8])?, ObjectId::from_bytes(&row[8..])?));
            }
            InodeNode::Branch { level, subtree_count: total, children }
        }
        _ => return Err(CoreError::InvalidRecord("compact inode role/level")),
    };
    if node_subtree_bytes(value) != total.checked_mul(81).ok_or(CoreError::LengthOverflow)? || encode_inode(&node)? != canonical {
...
pub enum DirectoryNode {
    ...
    Branch { level: u8, subtree_count: u64, subtree_bytes: u64, children: Vec<(CanonicalName, ObjectId)> },
}

pub fn encode_directory(node: &DirectoryNode) -> CoreResult<Vec<u8>> {
    let (role, level, count, total, bytes) = match node {
        DirectoryNode::Leaf(rows) => (1, 0, rows.len(), rows.len() as u64,
            rows.iter().map(|(name, _)| 10 + name.as_bytes().len() as u64).sum()),
        DirectoryNode::Branch { level, subtree_count, subtree_bytes, children } => (2, *level, children.len(), *subtree_count, *subtree_bytes),
    };
    validate_node_header(level, count)?;
    if role == 2 && (level == 0 || count == 0 || total < count as u64 || bytes < total.checked_mul(11).ok_or(CoreError::LengthOverflow)?) {
    ...
    match node {
        DirectoryNode::Leaf(rows) => {
            ...
            for (name, serial) in rows { put_bytes(&mut value, name.as_bytes())?; value.extend_from_slice(&serial.0.to_be_bytes()); }
        }
        DirectoryNode::Branch { children, .. } => {
            ordered(children.iter().map(|(name, _)| name.as_bytes()))?;
            for (name, id) in children { put_bytes(&mut value, name.as_bytes())?; value.extend_from_slice(id.as_bytes()); }
        }
    }
    finish_node(value)
}
...
pub fn decode_directory(canonical: &[u8]) -> CoreResult<DirectoryNode> {
    ...
    if count > (value.len() - 31) / 11 { return Err(CoreError::UnexpectedEof); }
    let node = match (value[10], value[11]) {
        ...
        (2, level) if level > 0 => {
            ...
            DirectoryNode::Branch { level, subtree_count: node_subtree_count(value), subtree_bytes: node_subtree_bytes(value), children }
        }
        _ => return Err(CoreError::InvalidRecord("compact directory role/level")),
    };
    if cursor != value.len() || encode_directory(&node)? != canonical { return Err(CoreError::InvalidRecord("compact directory canonical form")); }
    Ok(node)
}
...
mod tests {
    ...
    fn retained_offline_metadata_is_byte_identical() {
        ...
        assert_eq!(crate::object::references::referenced_objects(root).unwrap(), vec![decode_root(root).unwrap().inode_table]);
        for bytes in [include_bytes!("../../tests/fixtures/compact/inode-leaf.bin").as_slice(), include_bytes!("../../tests/fixtures/compact/inode-branch.bin").as_slice()] {
            let node = decode_inode(bytes).unwrap();
            assert_eq!(encode_inode(&node).unwrap(), bytes);
            let expected = match node { InodeNode::Leaf(rows) => rows.len() * 2, InodeNode::Branch { children, .. } => children.len() };
            assert_eq!(crate::object::references::referenced_objects(bytes).unwrap().len(), expected);
        }
        for bytes in [include_bytes!("../../tests/fixtures/compact/directory-leaf.bin").as_slice(), include_bytes!("../../tests/fixtures/compact/directory-branch.bin").as_slice()] {
            let node = decode_directory(bytes).unwrap();
            assert_eq!(encode_directory(&node).unwrap(), bytes);
            let expected = match node { DirectoryNode::Leaf(_) => 0, DirectoryNode::Branch { children, .. } => children.len() };
            assert_eq!(crate::object::references::referenced_objects(bytes).unwrap().len(), expected);
        }
    }
    #[test]
    fn compact_namespace_roundtrip_and_invalid_inputs() {
        ...
        let root = NamespaceRoot { profile_id: id, scope: id, root_inode: serial, inode_table: id };
        assert_eq!(encode_root(root).unwrap().len(), 129);
        assert_eq!(decode_root(&encode_root(root).unwrap()).unwrap(), root);
        let record = InodeRecordV1 { kind: InodeKind::RegularFile, namespace_ref_count: 2, content_root: id, metadata_root: id };
        let leaf = InodeNode::Leaf((1..=100).map(|n| (InodeSerial::new(n).unwrap(), record)).collect());
        let bytes = encode_inode(&leaf).unwrap();
        assert_eq!(bytes.len(), 8144);
        assert_eq!(decode_inode(&bytes).unwrap(), leaf);
        let branch = InodeNode::Branch { level: 1, subtree_count: 100, children: vec![(InodeSerial::new(100).unwrap(), id)] };
        assert_eq!(decode_inode(&encode_inode(&branch).unwrap()).unwrap(), branch);
        let names = DirectoryNode::Leaf(vec![(CanonicalName::from_bytes(b"a").unwrap(), serial), (CanonicalName::from_bytes(b"b").unwrap(), serial)]);
        assert_eq!(decode_directory(&encode_directory(&names).unwrap()).unwrap(), names);
        ...
        for n in 0..bytes.len() { assert!(decode_inode(&bytes[..n]).is_err()); }
        let mut invalid = bytes.clone(); invalid.push(0); assert!(decode_inode(&invalid).is_err());
        let mut invalid = bytes; invalid[13 + 23] ^= 1; assert!(decode_inode(&invalid).is_err());
    }
}
