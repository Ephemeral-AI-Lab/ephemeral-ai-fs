use super::build::add;
use super::edit::load_node;
use super::state::{ObjectRead, RopeCounters, Summary};
use crate::error::{CoreError, CoreResult};
use crate::file::extent::{ChildDescriptorV3, ExtentNodeV3, ExtentSliceV3};
use crate::object::ObjectId;

pub(super) fn validate_node<S: ObjectRead>(
    store: &S,
    expected: Summary,
    root: bool,
        ...
        ExtentNodeV3::Leaf { extents, .. } => {
            for batch in extents.chunks(64) {
                ...
                store.get_authenticated_payload_lengths_batch(&ids, |id, payload_length| {
                    ...
                    index += 1;
                    if payload_length as usize > crate::file::cdc::MAXIMUM_CHUNK_BYTES
                        || extent
                            .source_offset
...
pub(super) fn coalesce(extents: &mut Vec<ExtentSliceV3>) -> CoreResult<()> {
    let mut index = 1;
