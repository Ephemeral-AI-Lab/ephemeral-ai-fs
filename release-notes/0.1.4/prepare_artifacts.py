#!/usr/bin/env python3
"""Prepare/verify six source-only assets (Python 3.11+; caller owns measurement lock).

Only committed files are archived. The evidence bundle is a selection of tracked
reports/receipts, not every raw observation referenced by local absolute paths.
It preserves original source identities and does not represent a benchmark rerun.
No command creates a tag, release, binary, package or runtime image.
"""
...
EVIDENCE = (
    ...
    "docs/roadmap/0.1/0.1.4/checkpoint-evidence",
    "docs/roadmap/0.1/0.1.3/checkpoint-evidence",
...
def resolve(ref, candidate):
    ...
    if not candidate:
        ...
        if commit != tagged:
            raise ValueError("release archives must resolve to the actual v0.1.4 tag")
    manifest = tomllib.loads(git("show", commit + ":Cargo.toml").decode())
    if manifest["workspace"]["package"]["version"] != "0.1.4":
        raise ValueError("selected commit does not have workspace version 0.1.4")
    return commit
...
def asset_names(commit, candidate):
    stem = "layerfs-0.1.4" + ("-candidate-" + commit if candidate else "")
...
def validate(output, commit, candidate):
    ...
    return {"commit": commit, "candidate": candidate, "assets": 6,
            "source_members": len(source), "evidence_members": len(evidence),
            "validation": "PASS", "raw_evidence_scope": "selected tracked receipts; external raw references remain external"}


def main():
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("output", type=Path, help="fresh output directory (existing directory only with --verify)")
    parser.add_argument("--ref", default=TAG, help="git ref; default actual v0.1.4 tag")
