"""Tiny fixture check; caller may run under the shared measurement lock."""
...
with tempfile.TemporaryDirectory(prefix='layerfs-artifact-check-') as directory:
    root = Path(directory)
    repo = root / 'repo'
    repo.mkdir()
    module.REPO = repo
    ...
    git('tag', 'v0.1.4')
    assert module.resolve('refs/tags/v0.1.4', False) == commit
