#!/usr/bin/env python3
"""Prepare/verify six source-only v0.1.5 assets (Python 3.11+).
...
def resolve(ref, candidate):
    ...
    if not candidate:
        ...
        if commit != tagged:
            raise ValueError("release archives must resolve to the actual v0.1.5 tag")
    manifest = tomllib.loads(git("show", commit + ":Cargo.toml").decode())
    if manifest["workspace"]["package"]["version"] != "0.1.5":
        raise ValueError("selected commit does not have workspace version 0.1.5")
    return commit
...
def asset_names(commit, candidate):
    stem = "layerfs-0.1.5" + ("-candidate-" + commit if candidate else "")
...
def main():
    ...
    parser.add_argument("--ref", default=TAG, help="git ref; default actual v0.1.5 tag")
