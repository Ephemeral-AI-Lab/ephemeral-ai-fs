...
| 25 | #107 mechanism and measured ordinary-storage saving | **PASS — accepted as a successful result by explicit owner decision** (`owner-107-acceptance.json`). Measured and published verbatim: pack rows 3,457 → 1,058, SQLite pack overflow slack 2,142,701 → 1,222,187 B (−920,514 B, −43 %), apparent Store 83,525,632 → 82,583,552 B (−1.13 %), **allocated Store 83,935,232 → 83,951,616 B (+0.02 %)** because the live file's allocation residue grew 409,600 → 1,368,064 B. The self-declared ≥1.5 % allocated target was **not met numerically** and is superseded by the owner's acceptance, not re-based. | final treatment | [issue107-result.md](issue107-result.md), [issue107-declaration.md](issue107-declaration.md), `owner-107-acceptance.json`, `issue107-storage-full157/census.json`, superseded attempt `issue107-coalesce-full157/` |
...
| Issue | Outcome |
|---|---|
| #118 (umbrella) | **COMPLETE — all required rows terminal.** Every row is PASS, an explicit recorded WARN, an owner-WAIVED target, or accepted by explicit owner decision. Row 25's self-declared allocated-byte figure was missed numerically and is accepted by the owner's decision, with the measurement published verbatim. |
...
| #107 | Mechanism implemented and measured; pack-row, page-slack and apparent-byte reductions demonstrated on the matching history; the self-declared ≥1.5 % *allocated*-byte figure was missed numerically (0.02 % allocated / 1.13 % apparent) and is accepted by explicit owner decision, with the measurement, its cause and the failed prediction recorded. |
| #100 | Closed by explicit owner decision: ordinary full157 physical attribution, the owner-accepted #107 storage work, speed preserved on the final treatment (157/157, complete command 763.218 s) with same-Store verification against all 157 oracles and a matching read-only Git157 control. Closure records the residual storage distance to Git (83,951,616 B allocated versus live 56,197,120 B) as an accepted residual of the ordinary-write route, and the `>64 KiB` SmallContent dispatch item as not separately re-verified. |
| #110 | Closed: the current-source requalification record is complete and its follow-up work is owned elsewhere (the repaired history miss by #106, the owner-WAIVED cold target by #111, storage by #100/#107, the campaign by #102). |
| #106 / #109 / #111 / #113 | Closed with their evidence: `unrelated-500` repaired to 13.791443406 s < 15 s; the warm-cache optimization delivered with the cold milestone owner-WAIVED; the cold gap dispositioned by the same waiver plus the now-qualified default-budget 32,000-edit spill route; the page-size trade measured with a scoped, non-global recommendation and its unattributed costs handed to #114. |
| #117 | Closed by explicit owner decision: free space 337 GiB ≥ the 150 GB criterion, registered worktrees and 45 stale evidence directories removed as disclosed, Tasks 3/4 declined beyond the guarded runner-owned retention and Task 5 (`~/.codex`) declared out of scope, archive custody retained. |
| #102/#108/#112/#114 | Deduplicated into the single qualification above; no historical matrix rerun. Still open with their own remaining gates (campaign requalification, per-commit/per-edit means, optimization groups G0–G5, scattered-100 attribution). |
