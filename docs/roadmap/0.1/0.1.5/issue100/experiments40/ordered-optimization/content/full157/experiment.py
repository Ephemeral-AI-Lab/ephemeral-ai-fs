"""Actual157-state whole-file graph + native slice adapter Store, original IDs retained."""
...
SRC=HERE.parent.parent/'metadata/v2-157/candidate.sqlite';DST=HERE/'candidate.sqlite';EXPECTED='96cfd0ea89d7e3cfe85940ddbc863f1f417e86d8b86f6aee3cc0c79154b15825'
FIXTURE=pathlib.Path('/Users/yifanxu/Ephemeral-AI-Lab/layerfs-issue100-45mb-evidence/retained-full157-1/identity.json');REPO=pathlib.Path('/Users/yifanxu/Ephemeral-AI-Lab/deepseek-history-data/git-snapshots-157-v1/snapshots.git');INV=pathlib.Path('/Users/yifanxu/Ephemeral-AI-Lab/layerfs-issue100-40mb-full157/git-baseline/verify-pack.txt')
ATTR=pathlib.Path('/Users/yifanxu/Ephemeral-AI-Lab/layerfs-issue100-40mb-history-deltas/content/attribution.json')
def sha(p):
...
started=time.time();assert sha(SRC)==EXPECTED and not DST.exists();assert sha(FIXTURE)=='1021fc899d097d48dad864bc1b8e4cd804d3b38dfeca3a69b36587646f830b24'
...
assert len(native)==3221
small={r['git_oid']:bytes.fromhex(r['id']) for r in json.loads(ATTR.read_text())['rows']};assert len(small)==75398 and set(small.values())==old_content-native
fixture=json.loads(FIXTURE.read_text())['fixtures']['deepseek-full'];assert [s['index'] for s in fixture['states']]==list(range(1,158))
...
large={o:n for o,n in regular.items() if n>=131072};assert len(large)==523 and set(regular)==set(small)|set(large) and max(large.values())<=codec.MAX_RAW
git={}
...
# Verification ordering only: preserve physicalrows/policy, group slice reads by owner.
verification_rows=[row for row in rows if row[0] not in native]+sorted((row for row in rows if row[0] in native),key=lambda row:(coverage.get(row[0],(row[0],0,0))[0],row[0]))
for ordinal,(ident,n,record) in enumerate(verification_rows):
