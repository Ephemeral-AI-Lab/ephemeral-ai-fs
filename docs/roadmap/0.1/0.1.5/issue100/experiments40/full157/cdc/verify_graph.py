...
assert len(records)==3221 and len(selected)==1347
...
(OUT/'all-native-verification.json').write_text(json.dumps(dict(scope='All3221native canonical objects authenticated after1347graph substitution; cycles/strict pack chronology/role/length/depth/raw/per-chain work and singleton optional8lookup caps. Group and originalpack membership accounting checked. Does not run product visit_wave scratch ownership, reconstruct non-lock complete files, enforce unobserved original admission-batch remaining quotas or validate prospective physical formats.',summary=summary,store_sha256=BEFORE,source_family_sha256=sha(OUT/'family-result.json'),script_sha256=sha(Path(__file__)),records=rows,packs=packrows),indent=2));print(json.dumps(summary,indent=2))
