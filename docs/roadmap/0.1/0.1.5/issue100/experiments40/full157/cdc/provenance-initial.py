...
identity=json.loads((RUN/'identity.json').read_text());states=identity['fixtures']['deepseek-full']['states'];assert len(states)==157
...
for s in states:
 ...
 for line in manifest.read_text().splitlines():
  ...
  file=Path(s['input'])/'blobs'/oid;data=file.read_bytes();assert sha(file)==s['input_seal']['blobs/'+oid];seals[str(step)]=dict(path=str(file),sha256=sha(file),length=len(data),git_oid=oid)
 ...
 p=json.loads((RUN/f'deepseek-full/performance-step-{step}.json').read_text());ranges[step]=[r['physical_storage']['diag_selected_pack_last_id'] for r in p['receipts'] if 'physical_storage' in r];assert len(ranges[step])==2
mismatches=[]
