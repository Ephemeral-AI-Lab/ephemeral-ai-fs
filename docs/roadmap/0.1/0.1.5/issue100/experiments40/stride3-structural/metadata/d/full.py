"""Exact fixedD on all54 original namespace roots; append-only output artifacts."""
...
def main():
    ...
    assert json.loads(source_manifest.read_text())['deepseek-stride3/host-runtime/store.sqlite']==codec.SHA
    ...
    print('PASS all54namespace semantics + alldirectories',len(dir_cache),'uniqueinodes',len(serial),flush=True)
    content=[row for row in rows if row[0] not in raw];assert len(content)==61766
