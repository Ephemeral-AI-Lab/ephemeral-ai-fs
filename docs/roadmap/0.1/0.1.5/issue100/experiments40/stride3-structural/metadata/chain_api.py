...
if __name__=='__main__':
    ...
    for layout in ('full','delta'):
        ...
        assert sum(len(b) for p,b in packs)==result['layouts'][layout]['pack_bytes'] and len(locators)==result['metadata_objects']
    records={i:r for i,step,kind,base,r in record_ledger()}
    assert all(decode_record(i,records)==b for i,b in inv.items())
    assert len(root_map())==len(roots)==54
    print('PASS cached FULL/DELTAexports: all pack hashes/locators,allmetadataexactcanonicalreconstructions,54unchangedroots')
