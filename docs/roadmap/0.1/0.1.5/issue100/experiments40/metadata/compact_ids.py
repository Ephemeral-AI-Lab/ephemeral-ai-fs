...
def build_inventory(raw,roots):
    ...
    reverse={v:k for k,v in serial.items()};assert len(serial)==len(reverse)==17922
...
def main():
    ...
    for name,digest in manifest.items():assert base.sha(HERE/name)==digest
    assert base.sha(base.STORE)==base.SHA
    checks_fixture=fixture();raw,original_rows,roots,original_check=base.load()
    inventory,serial,allocator,scope,state_checks,dir_cache=build_inventory(raw,roots)
    content_rows=[row for row in original_rows if row[0] not in raw];locators,pack=base.pack_inventory(inventory,max(r[2] for r in original_rows))
    index=base.project_index('D',content_rows+locators)
    index_path=HERE/'D-index.sqlite';db=sqlite3.connect(index_path)
    ...
    names={name:dict(pages=pages,bytes=size,payload=payload,unused=unused) for name,pages,size,payload,unused in db.execute('select name,count(*),sum(pgsize),sum(payload),sum(unused) from dbstat group by name')}
    assert db.execute('pragma integrity_check').fetchone()[0]=='ok'
    assert db.execute('select scope,highwater from scope_allocator').fetchone()==(scope,allocator.highwater)
    db.close();index.update(index_bytes=names['objects']['bytes'],allocator=names['scope_allocator'],all_tables=names,database_bytes=index_path.stat().st_size,sha256=base.sha(index_path))
    subtotal=pack['pack_bytes']+index['index_bytes']+index['allocator']['bytes']
    control=json.loads((HERE/'result.json').read_text())['results']['C']
    assert [x['semantic_sha256'] for x in state_checks]==[x['semantic_sha256'] for x in json.loads((HERE/'roots.json').read_text())['C']]
    result=dict(treatment='D',protocol_sha256=base.sha(HERE/'protocol-compact-ids.md'),script_sha256=base.sha(pathlib.Path(__file__)),scope=scope.hex(),allocated_serials=allocator.highwater,diagnostic_mapping_sha256=hashlib.sha256(b''.join(k+v for k,v in sorted(serial.items()))).hexdigest(),mapping_purpose='old32-byte IDs to new canonical scopedserials: verification oracle only; not runtime dictionary',original_codec_gate=original_check,semantic_verified_states=len(state_checks),directory_objects_verified=len(dir_cache),fixture=checks_fixture,**pack,index=index,new_canonical_objects=sum(i not in raw for i in inventory),removed_original_objects=sum(i not in inventory for i in raw),metadata_plus_index_plus_allocator_bytes=subtotal,control_C_subtotal_bytes=control['metadata_plus_all_object_index_bytes'],incremental_saving_vs_C=control['metadata_plus_all_object_index_bytes']-subtotal,total_saving_vs_A=json.loads((HERE/'result.json').read_text())['results']['A']['metadata_plus_all_object_index_bytes']-subtotal,scope_allocator_logical_payload_bytes=40,source_sha256_before=base.SHA,source_sha256_after=base.sha(base.STORE),codec_calls=base.codec_calls,max_codec_workspace_bytes=base.max_workspace)
    assert result['source_sha256_after']==base.SHA
    for name,digest in manifest.items():assert base.sha(HERE/name)==digest
    (HERE/'result-D.json').write_text(json.dumps(result,indent=2)+'\n');(HERE/'roots-D.json').write_text(json.dumps(state_checks,indent=2)+'\n')
    print(json.dumps({k:result[k] for k in ('metadata_objects','canonical_bytes','pack_bytes','metadata_plus_index_plus_allocator_bytes','incremental_saving_vs_C','total_saving_vs_A')},indent=2))
