...
SOURCE=pathlib.Path('/Users/yifanxu/Ephemeral-AI-Lab/layerfs-issue100-40mb-full157/metadata')
sys.path.insert(0,str(SOURCE));import api as D;import codec
sys.path.insert(0,str(HERE.parent/'tools'));import matcher
...
def decode_record(identity,records):
    r=records[identity];assert r
    if r[0]==0:b=r[1:]
    else:
        assert r[0]==1 and 41<len(r)<=8193
        baseid=r[1:33];assert baseid!=identity and baseid in records
        br=records[baseid];assert br[0]==0,'base must be selected FULL (depth1)'
        base=br[1:];assert len(base)<=8192 and leaf(base) and codec.oid(base)==baseid
        b=matcher.replay(r,baseid,base);assert len(b)+len(base)<=16384 and leaf(b)
...
def main():
    ...
    inv,roots,content,scope,highwater=D.original_inventory();assert len(inv)==24748 and len(roots)==158
    ...
    for step,cohort in enumerate(cohorts):
        ...
        for groupids in grouped(inv,cohort):
            ...
            for i in groupids:
                ...
                if rank:
                    ...
                    if records[origin][0]!=0:row['status']='origin_selected_DELTA'
            ...
            for i in groupids:
                ...
                if selected[0]:
                    baseid=selected[1:33];assert records[baseid][0]==0 and first[baseid]<step;physical_bases.add(baseid);max_closure=max(max_closure,len(inv[i])+len(inv[baseid]))
        ...
        if(step+1)%20==0:print('encoded root',step+1,'/158; selected deltas',counters['status_selected_DELTA'],flush=True)
    assert len(records)==len(inv)==len(locators) and len(ledger)==8930
    ...
    result=dict(layouts=summaries,actual_pack_saving_vs_matched_FULL=summaries['full']['pack_bytes']-summaries['delta']['pack_bytes'],published_D_hashsorted_pack_bytes=input_result['pack_bytes'],difference_vs_published_D=input_result['pack_bytes']-summaries['delta']['pack_bytes'],metadata_objects=len(inv),groups=len(groups_ledger),canonical_bytes=sum(map(len,inv.values())),selected_full_objects=sum(r[0]==0 for r in records.values()),selected_delta_objects=sum(r[0]==1 for r in records.values()),selected_full_record_bytes=sum(len(r) for r in records.values() if r[0]==0),selected_delta_record_bytes=sum(len(r) for r in records.values() if r[0]),physical_FULL_bases=len(physical_bases),physical_FULL_base_record_bytes=sum(len(records[i]) for i in physical_bases),base_accounting='subset of selectedFULLobjects; countedonce,notoverheadaddedagain',unrooted_carried_metadata_objects=len(unrooted),unrooted_roles=dict(C.Counter(codec.role(inv[i]).rstrip(b'\0').decode() for i in unrooted)),maximum_canonical_closure=max_closure,maximum_depth=1 if physical_bases else 0,counters=dict(counters),verification=dict(all_objects_authenticated=len(reconstructed),verified_states=len(state_results),fixture=tests,canonical_inventory_byte_equal=True),scope=scope.hex(),highwater=highwater,source_manifest_sha256=sha(SOURCE/'manifest.json'),source_store_sha256_after=sha(codec.STORE),cache_sha256=sha(HERE/'cache.sqlite'),protocol_sha256=sha(HERE/'protocol.md'),script_sha256=sha(pathlib.Path(__file__)),matcher_hashes={p.name:sha(p) for p in (HERE.parent/'tools').iterdir() if p.is_file()},codec_calls=codec.codec_calls,max_codec_workspace_bytes=codec.max_workspace,elapsed_seconds=time.monotonic()-started,scope_note='Onefixeddepth1policy,samechronologicalgroups/packs;entireDcanonicalinventoryunchanged.NoStoreallocationorCommitclaim.')
    (HERE/'result.json').write_text(json.dumps(result,indent=2)+'\n')
    print(json.dumps({k:result[k] for k in ['actual_pack_saving_vs_matched_FULL','difference_vs_published_D','selected_full_objects','selected_delta_objects','physical_FULL_bases','maximum_canonical_closure','counters','elapsed_seconds']},indent=2),flush=True)
