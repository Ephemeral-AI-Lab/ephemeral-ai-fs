...
The user-approved sub400 plan now governs this milestone (a sound400–450ms result is acceptable, with exact reporting). Follow-upA is selected from measured130.870ms consumer authentication. Its typed owned-validation implementation and tests are recorded in `issue47-commit-sub400-plan.md`; it preserves the shared initialization/Commit queue, checked SQL consumer and finality boundaries.

The sub400 follow-upA is assessed in attempt12: create Commit636.195ms, consumer179.085ms, required spill authentication8.010ms, unchanged canonical totals and complete physical retirement. The practical400–450ms milestone is not met. This focused attempt stops at the agreed reassessment point; the detailed sub400 plan and results ledger identify the remaining structural critical path without launching further automatic tuning.
