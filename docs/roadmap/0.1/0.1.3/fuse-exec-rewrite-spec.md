...
Define same-mount advisory locking and mapping behavior for supported tools before claiming installer/editor compatibility. Retain current normal-open mapping support and document created DIRECT_IO-handle limitations. Do not enable writable mapping/writeback that can mutate outside the Commit admission cut without a safe drain or explicit Busy exclusion. A writable open/mapping lifetime that cannot be quiesced must prevent successful Commit rather than yield a torn snapshot. Avoid claiming full POSIX/Windows semantics from a tiny-file benchmark.

SDK mutations use the same supported handle/mapping exclusion when kernel state cannot be safely drained or invalidated; userspace ordering alone cannot stop a cache hit in the kernel. Preserve the existing projection pause/exclusion responsibilities. Required entry/attribute/page invalidation completes before SDK success and resumption of conflicting access. If invalidation fails after mutation, retain the exact result and fail the affected presentation; do not reopen with stale cache or blindly replay the edit.
...
Preserve the initial managed-Exec Busy policy. Additionally track mount ingress and writable-handle/mapping activity so an unmanaged writer cannot evade the Commit contract. Concurrent ordinary access is supported; uninterrupted concurrent writable Commit is not added here.
...
Release/forget/cancellation and required drain completion remain admissible through reserved control capacity. An incompatible already-open writable handle/mapping returns Busy before waiting for its lifetime to end. For otherwise quiesceable admitted work, retain the caller's finite deadline: deadline expiry before publication returns Busy/cancellation and safely restores Active admission after resolving admitted ownership. Never wait for a close that the admission barrier itself prevents. Publication/install uncertainty instead follows the retained-outcome rule below.
