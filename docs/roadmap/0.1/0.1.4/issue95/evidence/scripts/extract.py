...
root=pathlib.Path('/Users/yifanxu/Ephemeral-AI-Lab/layerfs-issue91-runs/r26-full-benchmark-final/performance')
...
for family in ['dedup_cdc_locality','dedup_cross_file']:
 for p in sorted((root/family).glob('*/perf.jsonl')):
  ...
  rows.append(row)
assert len(rows)==30
with (out/'r26-diagnostics.csv').open('x') as f:
 w=csv.DictWriter(f,fieldnames=rows[0]);w.writeheader();w.writerows(rows)
(out/'r26-diagnostics.json').write_text(json.dumps(rows,indent=2)+'\n')
