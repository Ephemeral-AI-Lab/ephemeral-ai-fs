...
r=pathlib.Path(__file__).parent;root=r.parent/'layerfs-issue95';old=root.parent/'layerfs-issue91-runs'
commands=json.loads((old/'r26-preparation/full-benchmark-commands.json').read_text())['commands'];image=(r/'c2-image-id.txt').read_text().strip();receipts=[]
for c in commands:
