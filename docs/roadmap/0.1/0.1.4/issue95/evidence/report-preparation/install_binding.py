#!/usr/bin/env python3
"""Install an already prospective binding without modifying existing campaign receipts."""
import fcntl
import hashlib
import json
import os
...
def main():
    ...
    with (Path(os.environ.get('TMPDIR','/tmp'))/'layerfs-infra-measurement.lock').open('a') as lock:
        ...
        receipt = {'schema':'issue95-prospective-binding-installation-v1',
                   ...
                   'scope':'Installed pre-collection declaration byte-identically; all existing raw files and ledgers unchanged; no derived projection needed.'}
