...
### Static native API sequence — pinned-source clarification

For encoding, reserve the fixed1,048,576-byte eight-byte-aligned caller workspace and initialize `ZSTD_initStaticCCtx`; check null/error returns. Reset session and parameters, then set exactly the same requested parameters as S2 through `ZSTD_CCtx_setParameter`: compressionLevel3, windowLog20, contentSizeFlag1, checksumFlag1, dictIDFlag0, nbWorkers0. Bind the raw prefix with `ZSTD_CCtx_refPrefix` (null/zero for FULL), reserve the checked `ZSTD_compressBound` output within the frozen frame bound, and call one-shot `ZSTD_compress2`. The target, prefix, output and context workspaces have disjoint live ownership through the call; destroy only caller-owned buffers, never call `ZSTD_freeCCtx` on static memory.

Do not replace this setter sequence with `getCParams`/`setCParams` derived from zero dictionary size or a lower window: that can change S2's automatic source/prefix-sensitive strategy and is a different treatment. Estimate APIs may be used as checked advisory bounds in preflight, but the public generic estimator resolves unknown source/zero dictionary inputs and can be conservative; a number above1MiB is not by itself proof that bounded32KiB inputs cannot run in1MiB. Conversely, the observed dynamic541,720-byte context is not a worst-case static proof. The fixed static workspace is the hard production ceiling, and source/parameter-regime compiled tests must establish whether all supported cases fit. A mandatory allocation error blocks the frozen implementation; no dynamic retry or hidden parameter change.

For decoding a nonempty PREFIX, **do not call `ZSTD_DCtx_refPrefix` and assume static DCtx makes it allocation-free**. In pinned1.5.7, that API invokes `ZSTD_DCtx_loadDictionary_advanced`, which constructs a heap DDict even when the content is borrowed. Use `ZSTD_estimateDCtxSize()` and `ZSTD_estimateDDictSize(prefix_length, ZSTD_dlm_byRef)`, checked and rounded for eight-byte alignment; their actual caller-owned capacities must sum to at most262,144 bytes. Initialize `ZSTD_initStaticDCtx` and `ZSTD_initStaticDDict(..., prefix, prefix_length, ZSTD_dlm_byRef, ZSTD_dct_rawContent)` in those separate live regions. Reset/set decoder parameters, validate the frame as below, and call `ZSTD_decompress_usingDDict`. FULL uses the static DCtx with no DDict. Empty raw prefix is equivalent to no prefix for this one-shot decode. Do not call `ZSTD_freeDDict`/`ZSTD_freeDCtx` on these static regions; release their caller allocations after use.

The raw prefix must remain alive/unmodified until both the static DDict and its decode are finished. Explicit `rawContent` avoids interpreting user bytes beginning with dictionary magic as a formatted dictionary. Reinitialize/rebind for each independent record. This introduces no trained/shared dictionary or persistent dictionary object: the temporary DDict is a bounded decoder handle over the authenticated base bytes. Its workspace is included in the256KiB decoder allowance and1MiB chain scratch, not added outside those caps. Exact parameter equivalence, checksum/size validation, static lifetime behavior and no hidden allocator calls require future compiled tests; no codec or test was run to make this source-only clarification.
...
|Native static decoder workspace|262,144 B maximum combined DCtx + by-reference raw static DDict capacities|
