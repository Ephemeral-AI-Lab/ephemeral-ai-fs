...
OUT=ROOT.parent/'g2-profile-10k-01'
...
cases=[('init_namespace','namespace-10000')]
OUT.mkdir()
...
def capture():
 ...
 while time.monotonic()<end:
  ...
  matched=[x.strip() for x in lines if str(ROOT/'target/release/fs-benchmark-pro')+' infra-run init_namespace namespace-10000 1 performance' in x]
  # The registered runner uses perform, not a wrapper/fixture command.
  if not matched:
   matched=[x.strip() for x in lines if str(ROOT/'target/release/fs-benchmark-pro')+' infra-run init_namespace namespace-10000 1 perform' in x]
  if matched:
   (OUT/'profile-process.txt').write_text('\n'.join(matched))
   pid=matched[0].split()[0];cmd=['sample',pid,'1','1','-file',str(OUT/'profile.txt')]
