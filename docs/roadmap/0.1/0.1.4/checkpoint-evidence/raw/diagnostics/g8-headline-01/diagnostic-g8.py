...
ROOT=pathlib.Path('/Users/yifanxu/Ephemeral-AI-Lab/layerfs-issue91-runs/source-g8a')
OUT=ROOT.parent/'g8-headline-01'
...
image_tag=(ROOT.parent/'build-g8a/image.log').read_text().strip().splitlines()[-1]
...
cases=[('init_namespace','namespace-100-compact-v3'),('payload_create_read','payload-create-100m'),
       ('init_namespace','namespace-100000'),('payload_create_read','payload-create-500m')]
OUT.mkdir()
identity=json.loads(pathlib.Path(args.host_binary+'.identity.json').read_text())
assert identity['LAYERFS_SOURCE_DIRTY']=='false'
assert identity['binary_sha256'] != json.loads((ROOT.parent/'source-g2/target/release/fs-benchmark-pro.identity.json').read_text())['binary_sha256']
...
(OUT/'declaration.json').write_text(json.dumps(dict(source=str(ROOT),host=identity,image=image,cases=cases,
 seed=1,diagnostic_only=True,amendment='b7e83574a',initialization_diagnostic_nonce='91',
