...
// The byte path is a positional argv operand, never executable shell text.
fn mounted_path(hex: &str) -> AnyResult<OsString> {
    if hex.is_empty() || hex.len() % 2 != 0 || !hex.bytes().all(|b| b.is_ascii_digit() || (b'a'..=b'f').contains(&b)) {
        return Err("lowercase path hex".into());
    }
    let raw = (0..hex.len()).step_by(2).map(|i| u8::from_str_radix(&hex[i..i+2],16)).collect::<Result<Vec<_>,_>>()?;
    if raw.contains(&0) || raw.split(|b| *b == b'/').any(|p| p.is_empty() || p == b"." || p == b"..") {
        return Err("canonical relative byte path".into());
    }
    let mut mounted = b"/workspace/storage-smoke/".to_vec();
    mounted.extend_from_slice(&raw);
    Ok(OsString::from_vec(mounted))
}
...
mod tests {
    ...
    #[test]
    fn path_is_literal_byte_argument() {
        assert_eq!(mounted_path("612f27ff24286229").unwrap().as_bytes(), b"/workspace/storage-smoke/a/\x27\xff$(b)");
        for bad in ["", "2f61", "612f", "612f2f62", "2e", "2e2e2f61", "6100", "FF", "0"] {
            assert!(mounted_path(bad).is_err(), "{bad}");
        }
    }
}
...
fn run() -> AnyResult<()> {
    ...
    let [root, container, row_path] = a.as_slice() else {
        return Err("ROOT CONTAINER SEALED_ROW_JSON".into());
    };
    if std::fs::metadata(row_path)?.len() > 65_536 { return Err("row input bound".into()); }
    let raw = std::fs::read(row_path)?;
    if raw.len() > 65_536 { return Err("row input bound".into()); }
    let row: Value = serde_json::from_slice(&raw)?;
    let text = |key: &str| -> AnyResult<&str> { row[key].as_str().ok_or_else(|| format!("row string {key}").into()) };
    let number = |key: &str| -> AnyResult<u64> { row[key].as_u64().ok_or_else(|| format!("row integer {key}").into()) };
    if text("schema")? != "issue88-depth-read-row-v1" { return Err("row schema".into()); }
    let branch = text("branch_id")?;
    let commit = text("commit_id")?;
    let row_id = number("row_index")?;
    let checkpoint = number("checkpoint")?;
    let depth = number("depth")?;
    let length = number("file_length_bytes")?;
    let offset = number("offset_bytes")?;
    let count = number("requested_bytes")?;
    let operation = text("operation")?;
    let digest = text("expected_sha256")?;
    if !(1..=157).contains(&checkpoint) || depth > 4 || !(4096..=8_388_608).contains(&length)
        || count == 0 || offset.checked_add(count).is_none_or(|end| end > length)
        || !matches!((operation, offset, count), ("range", _, 4096) | ("full", 0, _) )
        || (operation == "full" && count != length)
        || digest.len() != 64 || !digest.bytes().all(|b| b.is_ascii_hexdigit()) {
        return Err("row range/digest/shape".into());
    }
    let path = mounted_path(text("path_hex")?)?;
    ...
    let result = (|| -> AnyResult<()> {
        ...
        emit(
            "ready",
            json!({"setup_ns":u64::try_from(setup.elapsed().as_nanos())?,"setup_physical_storage":physical(store.physical_storage_receipt()),"setup_physical_scope":"absolute fresh StoreDb counters from open through fork/mount proof; before requested read","workspace_id":id.to_string(),"fork_branch_id":fork.to_string(),"checkpoint_index":checkpoint,"selected_depth":depth,"path_hex":row["path_hex"],"mount":"/workspace/storage-smoke","file_length_bytes":length,"offset_bytes":offset,"requested_bytes":count,"mount_proof":"PASS"}),
        )?;
        for phase in ["read", "digest"] {
            ...
            let shell=format!("/usr/bin/dd if=\"$1\" bs=65536 iflag=skip_bytes,count_bytes,fullblock skip={offset} count={count} status=none | {sink}");
            let argv = vec![
                "/bin/bash".into(),
                "-o".into(),
                "pipefail".into(),
            ...
            let correct = if phase == "read" {
                ...
                text.split_whitespace().next() == Some(digest)
            };
