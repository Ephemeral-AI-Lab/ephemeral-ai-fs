...
`layerfs --version` prints `layerfs 0.1.5`. This release has no compaction
...
0.1.5 does not forward the caller's PTY; use `workspace exec -- /bin/sh -c
...
Store retrieval is paged internally, but the 0.1.5 CLI drains every page into
one response. Use the SDK query cursor for caller-bounded consumption of large
entity sets. In 0.1.5, `--json` emits an envelope with `schema_version: 4`, but
