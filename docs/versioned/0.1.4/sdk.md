...
The documented Client entry points and CLI grammar remain available. The live
runtime architecture and concurrency behavior introduced in 0.1.3 remain in 0.1.4; old descriptions
of a host-only mutable Workspace or a fresh helper process per mount do not
apply to this release. Do not mix active runtime components from different
versions. See [container runtime](container-runtime.md), [storage](storage-format.md)
and the [qualification report](../../roadmap/0.1/0.1.4/issue95/README.md) for their separate
